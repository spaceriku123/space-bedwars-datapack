# summon
    summon item_display ~ ~1.2 ~ {width:0f,height:0f,teleport_duration:0,Tags:["shop","shop2_interaction"],Passengers:[{id:"minecraft:interaction",width:1f,height:0.7f,response:1b,Tags:["shop","shop2_interaction"],interaction:{timestamp:0}}]}
    summon item_display ~ ~1.2 ~ {view_range:0f,width:0f,height:0f,teleport_duration:0,Tags:["shop","shop2"],Passengers:[{id:"minecraft:chest_minecart",Invulnerable:1b,CustomDisplayTile:1b,Tags:["shop","shop2"],Passengers:[{id:"minecraft:marker",Tags:["shop","shop2"]}],CustomName:'"Team Upgrades & Traps"',DisplayState:{Name:"minecraft:air"}}]}
    ...
    execute at @s run scoreboard players operation @n[type=item_display,tag=shop2_interaction] player_id = @s player_id
    execute at @s run scoreboard players operation @n[type=marker,tag=shop2] player_id = @s player_id
    execute at @s run scoreboard players operation @n[type=interaction,tag=shop2_interaction] player_id = @s player_id
# tag
    tag @s add shopping2
