...
execute as @a[team=green] at @s run playsound entity.wither.death master @s ~ ~ ~ 0.5
execute at @e[tag=bed_green] run playsound entity.ender_dragon.ambient master @a[team=!green] ~ ~ ~ 0.5
...
title @a[team=green] subtitle {"text": "もう二度とリスポーンできません！","color": "white"}
title @a[team=green] title {"text": "ベッドが破壊された！","bold":true,"color": "red"}

execute as @a[predicate=main:destroy_bed] run tellraw @a ""
execute as @a[predicate=main:destroy_bed] run tellraw @a [{"text": "","color": "gray"},{"text": "BED DESTRUCTION > ","color": "white","bold": true},{"text": "Green Bed","color": "green"},{"text": "は "},{"selector": "@s"},{"text": " に破壊された！"}]
execute as @a[predicate=main:destroy_bed] run tellraw @a ""
